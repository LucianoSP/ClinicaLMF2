...
class RegistroGuia(BaseModel):
    ...
    def validar_linha(cls, v, values):
        tem_data = 'data_atendimento' in values and values['data_atendimento'] is not None
...
def processar_guia_medica(caminho_imagem: str, api_key: str) -> Dict:
    ...
    "registros": [
        {
            "data_atendimento": string,         // Formato: DD/MM/YYYY
    ...
    try:
        ...
        return {
            ...
            'resposta_raw': resposta.content[0].text if 'resposta' in locals() else None
        }
