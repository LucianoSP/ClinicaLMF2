...
export async function criarProcedimento(data: Omit<Procedimento, 'id' | 'created_at' | 'updated_at'>) {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/procedimentos`, {
        method: 'POST',
        headers: {
...
export async function atualizarProcedimento(id: string, data: Partial<Procedimento>) {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/procedimentos/${id}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'user-id': supabase.auth.getUser()?.data?.user?.id || '',
