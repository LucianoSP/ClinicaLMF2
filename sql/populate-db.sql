        ...
        FOR carteirinha IN SELECT * FROM carteirinhas WHERE paciente_id = paciente.id LOOP
            FOR i IN 1..5 LOOP
                INSERT INTO guias (
                    ...
                    'G' || carteirinha.numero_carteirinha || i,
                    'OP' || carteirinha.numero_carteirinha || i,
...
-- Inserir fichas de presença (5 por paciente)
DO $$
DECLARE
    paciente record;
    guia record;
...
BEGIN
    FOR paciente IN SELECT * FROM pacientes LOOP
        FOR guia IN SELECT * FROM guias WHERE paciente_id = paciente.id LIMIT 5 LOOP
            ...
            -- Inserir 10 sessões para cada ficha
            FOR i IN 1..10 LOOP
                INSERT INTO sessoes (
                    ficha_presenca_id,
                    data_sessao,
                    ...
                    i <= 4, -- primeiras 4 sessões têm assinatura
                    50.00,
                    CASE 
                        WHEN i <= 4 THEN 'conferida'::status_sessao
                        ELSE 'pendente'::status_sessao
                    END,
                    i <= 4, -- primeiras 4 sessões executadas
                    paciente.id
                );
            END LOOP;
        END LOOP;
...
-- Registrar execuções para as sessões executadas
