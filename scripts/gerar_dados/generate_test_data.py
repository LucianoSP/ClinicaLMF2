...
def gerar_dados_base():
    ...
    for nome in nomes:
        ...
        datas_disponiveis = [datetime(2024, 11, dia) for dia in range(18, 24)]
...
def gerar_dados_teste():
    ...
    try:
        ...
        # Prepara e salva execucaos
        print("\nInserindo execucaos com divergências...")
        execucaos_salvos = 0
        execucaos_erro = 0

        for i, data in enumerate(base_data):
            # 20% dos protocolos não terão execucao correspondente
            if i % 5 == 0:
                print(f"Pulando execucao {i+1} (20% sem correspondência)")
                continue

            # 15% dos execucaos terão data diferente do protocolo
            data_execucao = data["data_execucao"]
            if i % 7 == 0:
                ...
                if nova_data.day <= 23:  # Manter dentro do período válido
                    ...
                    print(f"execucao {i+1} com data diferente: {data_execucao}")

            # 10% dos execucaos não terão assinatura
            possui_assinatura = True
            if i % 10 == 0:
                possui_assinatura = False
                print(f"execucao {i+1} sem assinatura")

            # 10% dos execucaos não terão código de ficha
            codigo_ficha = f"F{random.randint(1000, 9999)}"
            if i % 10 == 0:
                codigo_ficha = ""
                print(f"execucao {i+1} sem código de ficha")
            ...
            # Salvar execucao
            print(f"\nSalvando execucao {i+1}:")
            print(f"Dados: {execucao}")
            if salvar_guia(execucao):
                execucaos_salvos += 1

                # Registrar divergências se houver
                if data["paciente_nome"] != paciente_nome:
                    print(
                        f"Registrando divergência de nome para guia {data['guia_id']}"
                    )
                    registrar_divergencia(
                        guia_id=data["guia_id"],
                        data_execucao=data_execucao,
                        codigo_ficha=codigo_ficha,
                        descricao=f"Nome do beneficiário divergente. No protocolo: {data['paciente_nome']}, no execucao: {paciente_nome}",
                        paciente_nome=data[
                            "paciente_nome"
                        ],  # Nome do paciente do protocolo
                    )

                if data["data_execucao"] != data_execucao:
                    print(
                        f"Registrando divergência de data para guia {data['guia_id']}"
                    )
                    registrar_divergencia(
                        guia_id=data["guia_id"],
                        data_execucao=data_execucao,
                        codigo_ficha=codigo_ficha,
                        descricao=f"Data de execução divergente. No protocolo: {data['data_execucao']}, no execucao: {data_execucao}",
                        paciente_nome=data[
                            "paciente_nome"
                        ],  # Nome do paciente do protocolo
                    )
                ...
                if not possui_assinatura:
                    print(
                        f"Registrando divergência de assinatura para guia {data['guia_id']}"
                    )
                    registrar_divergencia(
                        guia_id=data["guia_id"],
                        data_execucao=data_execucao,
                        codigo_ficha=codigo_ficha,
                        descricao="execucao sem assinatura do beneficiário",
                        paciente_nome=data[
                            "paciente_nome"
                        ],  # Nome do paciente do protocolo
                    )
                ...
                if not codigo_ficha:
                    print(
                        f"Registrando divergência de código de ficha para guia {data['guia_id']}"
                    )
                    registrar_divergencia(
                        guia_id=data["guia_id"],
                        data_execucao=data_execucao,
                        codigo_ficha=codigo_ficha,
                        descricao="execucao sem código de ficha",
                        paciente_nome=data[
                            "paciente_nome"
                        ],  # Nome do paciente do protocolo
                    )
            else:
                print(f"Erro ao salvar execucao {i+1}: {execucao}")
                execucaos_erro += 1

        print("\nDados de teste gerados com sucesso!")
        print(f"\nResumo:")
        print(f"- Total de protocolos gerados: {len(protocolos)}")
        print(f"- Total de execucaos salvos: {execucaos_salvos}")
        print(f"- Total de execucaos com erro: {execucaos_erro}")
        print(f"\nCenários de teste incluídos:")
        print("- 20% dos protocolos não têm execucao correspondente")
        print("- 15% dos execucaos têm data diferente do protocolo")
        print("- 10% dos execucaos não têm assinatura")
        print("- 10% dos execucaos não têm código de ficha")
        print("- 5% dos execucaos estão duplicados")
        print("- 50% dos execucaos têm nome e carteirinha diferentes")
        return True
