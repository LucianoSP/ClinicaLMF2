...
def verificar_datas(protocolo: Dict, atendimento: Dict) -> bool:
    ...
    try:
        ...
        data_atendimento = datetime.strptime(atendimento['data_atendimento'], '%d/%m/%Y')
        return data_protocolo == data_atendimento
    except ValueError as e:
        logging.error(f"Erro ao comparar datas: {e}")
        return False

def verificar_quantidade_atendimentos(protocolo: Dict, atendimentos: List[Dict]) -> bool:
...
def realizar_auditoria(data_inicial: str = None, data_final: str = None):
    ...
    for protocolo in protocolos:
        ...
        if protocolos_processados % 100 == 0:
            logging.info(f"Progresso: {protocolos_processados}/{total_protocolos} protocolos processados")
        ...
        atendimentos = listar_guias(
            limit=0,
            nome_beneficiario=protocolo['nomePaciente']
        )
        
        # Filtra atendimentos pelo número da guia
        atendimentos_correspondentes = [
            atend for atend in atendimentos['atendimentos']
        ...
        for atendimento in atendimentos_correspondentes:
            ...
            if not atendimento['possui_assinatura']:
                ...
                registrar_divergencia(
                    ...
                    data_exec=atendimento['data_atendimento'],
                    codigo_ficha=atendimento['codigo_ficha'],
                    descricao=f'Atendimento sem assinatura. Paciente: {atendimento["nome_beneficiario"]}'
                )
            ...
            if not atendimento['codigo_ficha']:
                ...
                registrar_divergencia(
                    ...
                    descricao=f'Atendimento sem código de ficha. Paciente: {atendimento["nome_beneficiario"]}'
                )
