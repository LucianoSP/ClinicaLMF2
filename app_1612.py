...
async def upload_pdf(
    ...
    results = []
    processed_files = set(
    )  # Para evitar processar o mesmo arquivo mais de uma vez
    ...
    for file in files:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Arquivo lido com sucesso. Tamanho: {len(content)} bytes")
                ...
                data_formatada = info["json"]["registros"][0][
                    "data_execucao"].replace("/", "-")
                ...
                if dados_guia["registros"]:
                    ...
                    for registro in dados_guia["registros"]:
                        ...
                        ficha_id = salvar_ficha_presenca({
                            ...
                            registro["paciente_carteirinha"],
                            "paciente_nome":
                            registro["paciente_nome"],
                            "numero_guia":
                            registro["guia_id"],
                            "codigo_ficha":
                            dados_guia["codigo_ficha"],
                            "possui_assinatura":
                            registro["possui_assinatura"],
...
async def upload_excel(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            colunas_necessarias = [
                "idGuia", "nomePaciente", "DataExec", "Carteirinha",
                "Id_Paciente"
            ]
            colunas_faltantes = [
...
async def clear_excel_data():
    ...
    try:
        ...
        if success:
            ...
            return {
                "success": True,
                "message": "Dados do Excel limpos com sucesso"
            }
        else:
            logger.error(
                "Erro ao limpar dados do Excel: operação retornou False")
            raise HTTPException(
                status_code=500,
                detail=
                "Erro ao limpar dados do Excel: operação não foi concluída com sucesso"
            )
    except Exception as e:
...
async def get_divergencias(
    ...
    status: str = Query(None,
                        description="Filtro por status (Pendente/Resolvido)"),
):
...
async def atualizar_divergencia(divergencia_id: int,
...
async def atualizar_atendimento_endpoint(codigo_ficha: str,
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=
                f"Erro ao atualizar atendimento no banco de dados: {str(e)}",
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro interno ao atualizar atendimento: {str(e)}")
...
async def excluir_atendimento(codigo_ficha: str):
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"Erro ao excluir atendimento: {str(e)}")
    finally:
        conn.close()
...
async def listar_fichas(
    ...
    try:
        result = listar_fichas_presenca(limit=limit,
                                        offset=offset,
                                        paciente_nome=paciente_nome)
        return result
    except Exception as e:
...
async def listar_divergencias_endpoint(
    ...
    try:
        ...
        if data_inicio:
            data_inicio = datetime.strptime(data_inicio,
                                            "%Y-%m-%d").strftime("%d/%m/%Y")
        if data_fim:
            data_fim = datetime.strptime(data_fim,
                                         "%Y-%m-%d").strftime("%d/%m/%Y")

        resultado = listar_divergencias(limit=limit,
                                        offset=offset,
                                        status=status)

        if resultado is None:
            raise HTTPException(status_code=500,
                                detail="Erro ao listar divergências")
