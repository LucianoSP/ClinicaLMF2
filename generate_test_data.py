from database import (
    init_db, limpar_banco, limpar_protocolos_excel, 
    salvar_guia, salvar_dados_excel
)
import sqlite3
...
def gerar_dados_base(quantidade=50):
    ...
    for nome in nomes:
        ...
        for _ in range(num_registros):
            ...
            data = (start_date + timedelta(days=random.randint(0, 5))).strftime('%d/%m/%Y')
...
def generate_test_data(quantidade=50):
    ...
    for i, data in enumerate(base_data):
        ...
        if i % 7 == 0:
            ...
            data_atendimento = (data_obj + timedelta(days=1)).strftime('%d/%m/%Y')
            print(f"Atendimento {i+1} com data diferente: {data_atendimento}")
        ...
        try:
            ...
            if i % 20 == 0:
                ...
                atendimento_duplicado['codigo_ficha'] = f"F{random.randint(1000, 9999)}"  # Código de ficha diferente
